...
experiment expFictitiousPlay type: gui {
	...
	parameter 'Agents\' strategy' var: peopleStrategy <- 'Fictious play' among: ['Pure','Fictious play'] category: 'Simulation mode';
	...
	output{
		/* If it is interesting for the model user to see sum_payoffs
		display chart_display refresh_every: 10 {
	    	chart "PayOff" type: series background: rgb("lightGray") size: {0.9, 0.4} position: {0.05, 0.05} {
	        	data "SumPayOff" value: sum(agents of_generic_species(peopleFictitiousPlay) collect(each.sumPayoffs));
	    	}
		}*/
		
		// Needed to be remodeled, because it is now possible to generate a non-uniform sample of agent.
		display Behav type: opengl{
			graphics 'G' {
				loop i from: 0 to: length(peopleFictitiousPlay)-1 {
					draw string(peopleFictitiousPlay(i).guiltAversion) at:{i*5, -5} color:rgb('black');
					draw string(peopleFictitiousPlay(i).guiltAversion) at:{-5, 5*i} color:rgb('black');
			...
			graphics 'G' {
				loop i from: 0 to: length(peopleFictitiousPlay)-1 {
					...
					loop j from: 0 to: length(peopleFictitiousPlay)-1 {
						...
						if(boundaries contains peopleFictitiousPlay(j).guiltAversion or boundaries contains peopleFictitiousPlay(i).guiltAversion){
							draw square(5) at:{i*5,j*5}
								color: rgb('white');
						}
						else{
