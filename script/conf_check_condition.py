...
def do_your_job(file_configurations_name, file_conditions_name):
	...
	for config in configurations:
		...
		for condition in conditions:
			try:
				if eval(condition): # condition is evaluated
					data.add_v(conf_sequence, str(condition)) # adding satisfied condition as a value of key configuration
					output.append("-> " + conf_sequence + tools.bcolors.OKGREEN + " |= " +  condition + tools.bcolors.ENDC + ".")
				else:
					output.append("-> " + conf_sequence + tools.bcolors.FAIL + " |/= " + tools.bcolors.OKBLUE + condition + tools.bcolors.ENDC + ".")					
			except:
